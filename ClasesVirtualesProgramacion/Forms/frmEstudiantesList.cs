    ...
    public partial class frmEstudiantesList : Form
    {
        ...
        private void frmEstudiantesList_Load(object sender, EventArgs e)
        {
            ...
            if (oConexion.SelectData(SelectSQL, dsClasesVirtuales.Estudiantes) != true)
                MessageBox.Show("No se ha podido cargar ningún dato de estudiantes, contacte el departamento de desarrollo técnico.", "Sin datos", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
        ...
        private void btnNuevo_Click(object sender, EventArgs e)
        {
            ...
            if (frmNuevo.DialogResult == DialogResult.OK)
            {
                ...
                if (oConexion.AccionSQL(sqlInsert) == true)
                {
                    ...
                    MessageBox.Show("La información de estudiante ha sido cerrada correctamente.", "Guardar", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        private void btnEditar_Click(object sender, EventArgs e)
        {
            if(estudiantesBindingSource.Count > 0)
            {
                ...
                if (frmEditar.DialogResult == DialogResult.OK)
                {
                    ...
                    {
                        this.frmEstudiantesList_Load(null, null);
                        MessageBox.Show("La información de estudiante ha sido actualizada correctamente.","Editar",MessageBoxButtons.OK, MessageBoxIcon.Information);
                        estudiantesDataGridView.Focus();
                    }
                }
            }
        ...
        private void btnEliminar_Click(object sender, EventArgs e)
        {
            if (estudiantesBindingSource.Count > 0)
            {
                if (MessageBox.Show("Asegurese de querer eliminar la información del estudiante. ¿Desea eliminar permanentemente este registro?", "Eliminar", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) == DialogResult.Yes)
                {
                    DataGridViewRow Fila = estudiantesDataGridView.CurrentRow;
                    Int16 ID = Int16.Parse(Fila.Cells[0].Value.ToString());
        ...
        private void btnBuscar_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Debe seleccionar una de las opciones para buscar un estudiante, ya sea por identidad, por nombres o por apellidos.", "Buscar", MessageBoxButtons.OK, MessageBoxIcon.Information);
                cmbBuscarPor.Focus();
                return;
            }
            else
